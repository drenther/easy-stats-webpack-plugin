...
function customOptionsMiddleware(customOptions) {
	if (customOptions.constructor === Array) {
		...
		if (validProps.length !== customOptions.length) {
			...
			warn('Refer to - https://github.com/drenther/easy-stats-webpack-plugin/README');
		}
...
function startRecursiveFilteringFromRoot(stats, options) {
	...
	filteredStats['assets'] = filterAssetsFields(filteredStats['assets'], options);
	filteredStats['chunks'] = filterChunksFields(filteredStats['chunks'], options);
	filteredStats['modules'] = filterModulesFields(filteredStats['modules'], options);
	return filteredStats;
}
